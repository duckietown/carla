...
Build instructions are available for Linux. You can also build CARLA in a Docker container for deployment in AWS, Azure or Google cloud services. Visit the [__Duckietown CARLA GitHub__](https://github.com/duckietown/carla) and clone the repository.
...
!!! note
    CARLA Duckietown is built and tested on Linux only. Upstream CARLA also documents a Windows build; if you need it, see the [upstream Windows instructions](https://carla.readthedocs.io/en/latest/build_windows/), bearing in mind that they do not cover the Duckietown repository or content.
